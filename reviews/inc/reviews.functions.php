...
function cot_reviews_list($userid, $area, $code = '', $name = '', $params = '', $tail = '', $showall = false)
{
    ...
    while ($item = $sql->fetch()) {
        ...
        // Форма редактирования для каждого отзыва то есть может изменить пользователя автора отзыва
        if ($usr['id'] == $item['item_userid'] || $usr['isadmin']) {
			 ...
			 */
			// тут админ редактирует только от именни заказчиков, то есть может изменить пользователя автора отзыва 
            $user_options = [];
            if ($usr['isadmin']) {
            ...
            // Загружаем данные пользователя явно
            $owner = $db->query("SELECT * FROM $db_users WHERE user_id = ?", [$item['item_userid']])->fetch();
            if ($owner) {
                $t1->assign(cot_generate_usertags($owner, 'EDIT_FORM_OWNER_'));
            } else {
                $t1->assign([
                    'EDIT_FORM_OWNER_ID' => $item['item_userid'],
                    'EDIT_FORM_OWNER_NAME' => htmlspecialchars($item['user_name'] ?: 'Неизвестный'),
                ]);
            }
            ...
            $t1->assign([
                ...
                'EDIT_FORM_SCORE' => cot_radiobox($item['item_score'], 'rscore', $L['reviews_score_values'], $L['reviews_score_titles'], 'class="form-control"'),
    ...
    if ($usr['auth_write'] && $usr['id'] != $userid) {
        ...
        /* 
		//тут админ добавляет от именни любого пользователя, то есть может указать пользователя автора отзыва
        $user_options = [];
        if ($usr['isadmin']) {
            $users_sql = $db->query("SELECT user_id, user_name FROM $db_users ORDER BY user_name ASC");
            while ($u = $users_sql->fetch()) {
		...
		// тут админ добавляет только от именни заказчиков, то есть может указать пользователя автора отзыва 
        $user_options = [];
        if ($usr['isadmin']) {
            $users_sql = $db->query("SELECT user_id, user_name FROM $db_users WHERE user_maingrp = 7 ORDER BY user_name ASC");
            while ($u = $users_sql->fetch()) {
        ...
        if (cot_module_active('projects') && $cfg['plugin']['reviews']['checkprojects'] && $usr['id'] > 0) {
            ...
            $prjreviews_string = count($prjreviews) > 0 ? "AND o.offer_pid NOT IN (" . $db->quote($prjreviews) . ")" : '';
        ...
        if ($usr['auth_write']) {
            ...
            $t1->assign([
                ...
                'ADD_FORM_SCORE' => cot_radiobox(0, 'rscore', $L['reviews_score_values'], $L['reviews_score_titles'], 'class="form-control"'),
...
function cot_reviews_last($count = 0)
{
    ...
    while ($item = $sql->fetch()) {            
        // Загружаем данные автора отзыва
        $owner = $db->query("SELECT * FROM $db_users WHERE user_id = ?", [$item['item_userid']])->fetch();
        if ($owner) {
            $t1->assign(cot_generate_usertags($owner, 'REVIEW_ROW_OWNER_'));
        } else {
            $t1->assign([
                'REVIEW_ROW_OWNER_ID' => $item['item_userid'],
                'REVIEW_ROW_OWNER_NAME' => htmlspecialchars($item['user_name'] ?: 'Неизвестный'),
                'REVIEW_ROW_OWNER_FULL_NAME' => htmlspecialchars($item['user_name'] ?: 'Неизвестный'),
            ]);
        }

        // Загружаем данные получателя отзыва
        $touser = $db->query("SELECT * FROM $db_users WHERE user_id = ?", [$item['item_touserid']])->fetch();
        if ($touser) {
            $t1->assign(cot_generate_usertags($touser, 'REVIEW_ROW_TO_'));
        } else {
            $t1->assign([
                'REVIEW_ROW_TO_ID' => $item['item_touserid'],
                'REVIEW_ROW_TO_NAME' => 'Неизвестный',
                'REVIEW_ROW_TO_FULL_NAME' => 'Неизвестный',
            ]);
        }
